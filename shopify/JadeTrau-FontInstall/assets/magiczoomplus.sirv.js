...
function mtInitSirvIcon(spinURL) {
    ...
    if (jQuery('.MagicToolboxSelectorsContainer a').length == 0) {
      ...
      jQuery('div[data-slide-id="zoom"]').addClass('active-magic-slide');
      Sirv.start();
...
function mtGetVariantId() {
  ...
  if(variantId == '' || variantId == undefined || variantId == null){
   ...
   var variantId =  jQuery("#product-selector").find("option:first-child").val(); 

  } else{
     var variantId =  jQuery("#idtest").val();
  }
...
function initmzp() {
    ...
    //mtInitVariants();
     ...
     //  currentVariantID = jQuery(".single-option-selector").val();
      var currentVariantID =  jQuery("input[name='id']").val();
    ...
    //  currentVariantID = $id; 
      ...
      //  $s.trigger('click')
       ...
       jQuery("#product-selector").val($id);
