...
//                 }
    ...
    _ajaxsidebar: function() {
      $(".sidebar-tag a, .sidebar-tag label").click(function(n) {
        ...
        try {
          ...
          if(tag != undefined) {
            if(Shopify.queryParams.constraint != '' && Shopify.queryParams.constraint != undefined) {
          ...
          $(".sidebar-tagm1 a, .sidebar-tagm1 label").click(function(n) {
        ...
        try {
          ...
          n.preventDefault();
              ...
              $(".sidebar-tagm2 a, .sidebar-tagm2 label").click(function(n) {
    ...
    $('.addlabel ul li  a').each(function() { 
	return;
        var filterTag = $(this);
      ...
      /*  $('.addlabel a').each(function() { 
       ...
       filterTag.on('click', function() {
        ...
        //    params.splice(params.indexOf(tag), 1);
         ...
         if(params.length <= 1){
        Shopify.queryParams.constraint = params.toString();
         }else{
              Shopify.queryParams.constraint = params.join('+');  
         }  
...
//       }
        ...
        $("#product-add-to-cart").click(function(n) {
          ...
          if ($(this).attr("disabled") != "disabled") {
            if (!window.ajax_cart) {
              ...
              var custom_text = $("#add-to-cart-form .engraving_input").val(); 
              
              var o = $("#product-featured-image").attr("src");
              mt._ajaxaddtocart(cart, i, o, custom_text)
            }
          }
          return false
        })
        ...
        $(".add-to-cart-btn").click(function(n) {
          ...
          if ($(this).attr("disabled") != "disabled") {
            ...
            if (!window.ajax_cart) {
              ...
              var custom_text = $("#add-to-cart-form .engraving_input").val(); 
              mt._ajaxaddtocart(s, o, u, custom_text)
            }
    ...
    _ajaxaddtocart: function(n, r, title, image) {
      ...
      console.log("custom Text", image);
      if(image != null){
          // add a key/value pair
          dataJson.properties.CustomText = image;
      }
    ...
    free_shipping: function(n) {
      ...
      if(price_pase < window.calculte_free_shipping_number){
        ...
        $(".cart-congrats .moneys").append(Shopify.formatMoney(tog_pr, window.money_format));
...
jQuery(".twelverange").find("input").attr("maxlength",12);
       jQuery(".twelverange").find("input").attr("minlength",1);
      
     jQuery(".threerange").find("input").attr("maxlength",3);
       jQuery(".threerange").find("input").attr("minlength",1);
      
     jQuery(".fourrange").find("input").attr("maxlength",4);
      jQuery(".fourrange").find("input").attr("minlength",1);
  
  $(document).ready(function(){
    /*
    jQuery(".twelverange").on("keyup change",function(e){
      var newselectted_val = 12;
    ...
    jQuery(this).find("input[type='text']").attr("maxlength",newselectted_val);
          if (jQuery(this).find("input").val() >= newselectted_val) {                         
           e.preventDefault();                
        }
        if(jQuery(this).find(".mw-option__helptext").hasClass("mw-hidden")) {
   ...
   jQuery(".threerange").on("keyup change",function(){
          var newselectted_val = 3;
      if (jQuery(this).find("input").val() >= newselectted_val) {               
           e.preventDefault(); 
   ...
   jQuery(".fourrange").on("keyup change",function(){
          var newselectted_val = 4;
       if (jQuery(this).find("input").val() >= newselectted_val) {                        
           e.preventDefault(); 
...
if(isMobile){
   ...
   /* jQuery(".twelverange").on("keyup change",function(e){
    ...
    jQuery(this).find("input[type='text']").attr("maxlength",newselectted_val);
       ...
       */
    //   jQuery(".threerange").find("input").hide();
   jQuery(".twelverange").find("input").attr("maxlength",12);
       jQuery(".twelverange").find("input").attr("minlength",1);
     ...
     jQuery(".fourrange").find("input").attr("maxlength",4);
      jQuery(".fourrange").find("input").attr("minlength",1);
       ...
       jQuery(".customval").find("input").on("keyup change",function(e){
         var selectted_values =  jQuery(this).find("input").attr("maxlength");
            if(selectted_values != undefined){
	   var current_val = jQuery(this).val();
          if(jQuery(this).val() != '' && parseInt(jQuery(this).val().length) > parseInt(selectted_values)){
                 e.preventDefault();
            return false;
          }
            }
