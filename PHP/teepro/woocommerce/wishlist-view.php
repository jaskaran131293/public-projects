	...
	<table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr($wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
        ...
        <tr>
            ...
            <th class="product-name">
                <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product Name', 'teepro' ) ) ?></span>
            </th>
            ...
            <?php if( $show_price ) : ?>
                ...
                <th class="product-price">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit Price', 'teepro' ) ) ?>
                    </span>
                </th>
            ...
            <?php if( $show_stock_status ) : ?>
                ...
                <th class="product-stock-status">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock Status', 'teepro' ) ) ?>
                    </span>
                </th>
        ...
        if( count( $wishlist_items ) > 0 ) :
                ...
                if( $product && $product->exists() ) :
                    ...
                    <tr id="yith-wcwl-row-<?php echo esc_attr($item['prod_id']); ?>" data-row-id="<?php echo esc_attr($item['prod_id']); ?>">
                        ...
                        <td class="product-remove">
                            <div>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php _e( 'Remove this product', 'teepro' ) ?>">&times;</a>
                            </div>
                        </td>
                        <?php endif; ?>
                        ...
                        <?php if( $show_price ) : ?>
                            <td class="product-price">
                                <?php print( $product->get_price() ) ? $product->get_price_html() : apply_filters( 'yith_free_text', __( 'Free!', 'teepro' ) ); ?>
                            </td>
                        <?php endif ?>

                        <?php if( $show_stock_status ) : ?>
                            <td class="product-stock-status">
                                <?php print($stock_status == 'out-of-stock') ? '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'teepro' ) . '</span>' : '<span class="wishlist-in-stock">' . __( 'In Stock', 'teepro' ) . '</span>'; ?>
                            </td>
                        <?php endif ?>
                        ...
                        <td class="product-add-to-cart">
	                        ...
	                        if( $show_dateadded && isset( $item['dateadded'] ) ):
								echo '<span class="dateadded">' . sprintf( __( 'Added on : %s', 'teepro' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
	                        endif;
	                        ?>
	                        ...
	                        <select class="change-wishlist selectBox">
		                        <option value=""><?php _e( 'Move', 'teepro' ) ?></option>
		                        <?php
		                        ...
		                        ?>
			                        <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
				                        ...
				                        if( $wl['wishlist_privacy'] == 1 ){
					                        $wl_privacy = __( 'Shared', 'teepro' );
				                        }
				                        elseif( $wl['wishlist_privacy'] == 2 ){
					                        $wl_privacy = __( 'Private', 'teepro' );
				                        }
				                        else{
					                        $wl_privacy = __( 'Public', 'teepro' );
				                        }
	                        ...
	                        <?php if( $is_user_owner && $repeat_remove_button ): ?>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php _e( 'Remove this product', 'teepro' ) ?>"><?php _e( 'Remove', 'teepro' ) ?></a>
                            <?php endif; ?>
                        </td>
	                <?php endif; ?>
                    </tr>
            ...
            <tr>
                <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products were added to the wishlist', 'teepro' ) ) ?></td>
            </tr>
        <?php
        endif;
	        ...
	        <td colspan="<?php echo esc_attr( $column_count ) ?>">
	            <?php if( $show_cb ) : ?>
		            <div class="custom-add-to-cart-button-cotaniner">
		                <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_token ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', __( 'Add the selected products to the cart', 'teepro' ) ) ?></a>
		            </div>
	            <?php endif; ?>
	            ...
	            <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
		            <div class="ask-an-estimate-button-container">
	                    ...
	                    <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', __( 'Ask for an estimate', 'teepro' ) ) ?>
	                </a>
		            </div>
	            <?php endif; ?>
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo esc_attr($ask_estimate_url); ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if( ! is_user_logged_in() ): ?>
				<label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', __( 'Your email', 'teepro' ) ) ?></label>
